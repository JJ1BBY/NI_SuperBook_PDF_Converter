...
{
    // サーバープロセス常駐化用フィールド (改修1)
    Process? _serverProcess;
    StreamWriter? _serverStdin;
    StreamReader? _serverStdout;
    AiUtilRealEsrganPerformOption? _serverOption;
    ...
    bool IsSameServerOption(AiUtilRealEsrganPerformOption a, AiUtilRealEsrganPerformOption b)
        => a.Model == b.Model && a.Tile == b.Tile && a.Pad == b.Pad &&
           Math.Abs(a.OutScale - b.OutScale) < 0.001 && a.FaceMode == b.FaceMode && a.Fp32 == b.Fp32;

    async Task EnsureServerStartedAsync(AiUtilRealEsrganPerformOption option, CancellationToken cancel)
    {
        if (_serverProcess != null && !_serverProcess.HasExited &&
            _serverOption != null && IsSameServerOption(_serverOption, option))
            return;

        await StopServerAsync();

        string additionalStr = "";
        if (option.FaceMode) additionalStr += " --face_enhance";
        if (option.Fp32) additionalStr += " --fp32";

        // venv の Python exe を直接呼び出す (activate 不要)
        string pythonExe = PP.Combine(this.BaseDirPath, @"venv\Scripts\python.exe");
        string scriptArgs = $"Real-ESRGAN/inference_realesrgan_server.py -n {option.Model} --tile {option.Tile} --tile_pad {option.Pad} --outscale {option.OutScale:F2}{additionalStr}";

        var psi = new ProcessStartInfo(pythonExe, scriptArgs)
        {
            WorkingDirectory = this.BaseDirPath,
        ...
        proc.ErrorDataReceived += (s, e) => { if (e.Data != null) Con.WriteLine($"[Real-ESRGAN Server] {e.Data}"); };
        proc.Start();
        proc.BeginErrorReadLine();

        Con.WriteLine($"[Real-ESRGAN Server] Starting server: {pythonExe} {scriptArgs}");

        // SERVER_READY を受信するまで待つ (最大 5 分)
        while (true)
        {
            cancel.ThrowIfCancellationRequested();
            var readTask = proc.StandardOutput.ReadLineAsync();
            if (await Task.WhenAny(readTask, Task.Delay(300_000, cancel)) != readTask)
            {
                try { proc.Kill(); } catch { }
                proc.Dispose();
                cancel.ThrowIfCancellationRequested();
                throw new TimeoutException("Real-ESRGAN server startup timed out (5 min).");
            }
            string? line = await readTask;
            if (line == null)
            {
                proc.Dispose();
                throw new CoresException("Real-ESRGAN server exited before becoming ready.");
            }
            Con.WriteLine($"[Real-ESRGAN Server] {line}");
            if (line == "SERVER_READY") break;
        }

        _serverProcess = proc;
        _serverStdin   = proc.StandardInput;
        _serverStdout  = proc.StandardOutput;
    ...
    async Task StopServerAsync()
    {
        ...
        try
        {
            if (!proc.HasExited)
            {
                try { await stdin!.WriteLineAsync("EXIT"); await stdin.FlushAsync(); } catch { }
                try { await proc.WaitForExitAsync().WaitAsync(TimeSpan.FromSeconds(5)); } catch { }
    ...
    async Task PerformInternalAsync(AiUtilRealEsrganPerformOption option, int timeout, CancellationToken cancel)
    {
        ...
        await EnsureServerStartedAsync(option, cancel);

        try
        {
            await _serverStdin!.WriteLineAsync($"{pythonBatchInDir}|{pythonBatchOutDir}");
            await _serverStdin.FlushAsync();

            // DONE / ERROR が届くまで読む
            while (true)
            {
                var readTask = _serverStdout!.ReadLineAsync();
                if (await Task.WhenAny(readTask, Task.Delay(timeout, cancel)) != readTask)
                {
                    cancel.ThrowIfCancellationRequested();
                    throw new TimeoutException($"Real-ESRGAN server job timed out ({timeout / 1000}s).");
                }
                string? line = await readTask;
                if (line == null) throw new CoresException("Real-ESRGAN server stdout closed unexpectedly during job.");
                if (line == "DONE") return;
                if (line.StartsWith("ERROR:")) throw new CoresException($"Real-ESRGAN: {line}");
                Con.WriteLine($"[Real-ESRGAN Server] {line}");
            }
        }
        catch
        {
            // エラー発生時はサーバーをリセット（次のリトライで再起動）
            await StopServerAsync();
            throw;
        }
    }
