...
public class ImageMagickUtil
{
    ...
    public async Task<int> GetPdfPageCountAsync(string pdfPath, CancellationToken cancel = default)
    {
        var result = await RunMagickAsync($"identify {pdfPath._EnsureQuotation()}", cancel: cancel);
        int count = result.OutputStr.Split('\n', StringSplitOptions.RemoveEmptyEntries).Length;
        return Math.Max(count, 1);
    }

    // ImageMagick のページ範囲指定を使って N 並列で PDF を展開する (改修2)
    public async Task ExtractImagesFromPdfParallelAsync(string pdfPath, string dstDir, ImageMagickExtractImageOption? option = null, CancellationToken cancel = default)
    {
        option ??= new();

        // デバッグ用の少ページ指定や既に小さい場合は通常処理にフォールバック
        bool limitedPages = option.NumPages >= 1 && option.NumPages != int.MaxValue;
        if (limitedPages)
        {
        ...
        string ext = option.Format == ImageMagickExtractImageFormat.Png ? ".png" : ".bmp";
        string bmpOptions = option.Format == ImageMagickExtractImageFormat.Png ? "" : "-depth 8 -type TrueColor BMP3:";
