...
- **RealESRGAN Python サーバスクリプトの GPU 処理高速化:** `inference_realesrgan_server.py` を以下の 2 点で最適化しました。
  ...
  2. **`torch.compile` + ウォームアップ:** PyTorch の JIT コンパイラ (`torch.compile(mode="max-autotune")`) でモデルの畳み込み演算を GPU カーネルに融合。コンパイル結果はスクリプト横の `.torch_compile_cache/` に永続キャッシュされ、2 回目以降の起動では再コンパイル不要です。コンパイルはサーバ起動時 (`SERVER_READY` 送信前) に完了させるため、1 冊目の 1 ページ目から最高速で処理されます。
...
- **VRAM 容量が小さい GPU (8 GB 級など):** Tile=2048 のままではメモリ不足 (OOM) で失敗する可能性が高いです。その場合は、`internal_libs/.../AppLib/Misc/AiUtil.cs` の `AiUtilRealEsrganPerformOption.Tile` の値を **1024 や 512 などに下げて**ください。タイルサイズを下げると VRAM 使用量は減りますが、タイル分割数が増えるぶん処理速度は低下します (上表の「約 7.9 倍」は得られなくなります)。
