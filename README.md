...
実運用 (35 冊規模の本番スキャン PDF 一括変換) で判明した問題に対し、以下の防御的修正を行ないました。
...
- **RealESRGAN Python サーバスクリプトの GPU 処理高速化:** `inference_realesrgan_server.py` を以下の 2 点で最適化しました。
  1. **I/O パイプライン化:** 先読みスレッド (`cv2.imread`) / GPU 推論メインスレッド / 後書きスレッド (`cv2.imwrite`) を 3 段パイプラインで並列実行。GPU が画像 I/O を待たずに推論専念できるため、1 枚あたりの処理時間が短縮されます。
  2. **`torch.compile`:** PyTorch の JIT コンパイラによる畳み込みカーネル融合を試みましたが、Windows 版 PyTorch nightly では Triton が `LoweringException` を送出し、Real-ESRGAN の `tile_process` 内の `except RuntimeError` が捕捉できず処理が失敗するため、現在は**無効化**しています。Linux 環境や将来の PyTorch バージョンでは有効化できる可能性があります。
...
#### 鮮明化 (RealESRGAN) 処理速度のオリジナル版との比較

処理時間の大半を占める RealESRGAN 画像鮮明化について、オリジナル版の既定設定 (モデル `x4plus` / Tile=512) から、本派生版の設定 (モデル `x2plus` / Tile=2048) へ変更したことで、**約 7.9 倍の高速化**を実測しました。
...
計測に使用した本環境は以下のとおりです (計測日: 2026/06/10)。

- **GPU:** NVIDIA GeForce RTX 5070 Ti (VRAM 16 GB)
- **CPU:** AMD Ryzen 7 5700X (8 コア)
- **メモリ:** 32 GB

> 上記の高速化倍率・秒数は本環境での実測値であり、GPU・CPU・メモリ構成によって変動します。
