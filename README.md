...
2026/06/13 JJ1BBY (オリジナル: 2026/01/22 登 大遊)

---

> ### 📦 本リポジトリは派生版 (derivative work) です
>
> 本リポジトリ **NI_SuperBook_PDF_Converter** は、[登 大遊 (dnobori) 氏によるオリジナル「DN_SuperBook_PDF_Converter」](https://github.com/dnobori/DN_SuperBook_PDF_Converter) を元に開発した派生版 (derivative work) です。なお、これは GitHub の Fork 機能による派生 (upstream と関連付けられたリポジトリ) ではなく、オリジナル版のソースを引き継いで**独立したリポジトリ**として公開しているものです。原作者の README「[8. 拡張開発されたい方へ](#8-拡張開発されたい方へ)」に記載された方針 (プルリクエストではなく、各自で派生プログラムを独自に開発・公開する) に従っています。
>
> - **派生版の保守・追加開発:** JJ1BBY
> - **オリジナル版 (本家):** https://github.com/dnobori/DN_SuperBook_PDF_Converter （原作者: 登 大遊 氏）
> - **派生版での追加・変更点・謝辞:** 下記「**1.4. 本派生版 (NI_SuperBook_PDF_Converter / JJ1BBY) における変更点**」をご覧ください。
>
> 以下、「1.4.」節を除く本文は、オリジナル版の設計・機能解説として、原作者 (登 大遊 氏) の文章をそのまま収録しています。
...
3. **2026/06/13 v2.1 不具合修正・機能追加**  
   縦書き (`right2left`) OCR 実行時に figure_merge が `md_paged` パスへ適用されなかった不具合を修正。ローカル環境固有の設定を `appsettings.local.json` で上書きできるよう対応 (リポジトリに影響しない)。EPUB 縦書き図版の SVG 回転処理を修正。近接図版マージ (figure_merge) が C# 側のパラメータ未渡しにより実際には動作していなかった不具合を修正 (`MergeFigureGapPx=100` を job JSON に反映)。
...
## 1.2. 本ソフトウェアによるスキャン PDF の紙の汚れ、裏写り、インクのにじみ、JPEG のモアレノイズの鮮明化機能の実行結果
...
## 1.3. 本ツールの目的 - 紙の書籍スキャン PDF を極めて読みやすくし、ストレスをゼロに近付けるとともに、高精細日本語 AI OCR を自動で実施する
...
8. **日本語 OCR において誤字・脱字が多く生じること。また、折角苦労して、物理／論理ページ番号や縦書きフラグ等を設定した PDF を、OCR ソフトにかけると、それらの苦労して作ったメタデータが全部消え、労力が水の泡になること**  
   PDF を OCR 処理し、全文検索したり、コピー・アンド・ペースト可能にしたりする場合、従来の OCR は、誤字・脱字がかなりあり、実用的とはいえませんでした。

   しかも、多くの PDF 対応の OCR ソフトウェアは、上記の物理／論理ページ番号や縦書きフラグ等を設定した PDF をかけると、一応、検索可能 PDF を作ってくれるのですが、元のこれらの苦労して作ったメタデータが全部消え、労力が水の泡になってしまいます。

   さらに、OCR 処理された結果を、フルテキストインデックス作成処理または AI 学習処理等のために、テキストデータに変換する際は、さらに一苦労します。たいていの場合、構造化されていない単なる文字データが出力されるだけです。構造をある程度反映した OCR 結果を、PDF だけでなく、HTML / Markdown / JSON 等で出力することが必要でしたが、これらは、既存の一般的なツールでは実現できないか、追加的な高度複雑なプログラミングが必要でした。
...
8. **高精細な日本語 OCR 処理を実施し、「文字埋め込み・検索可能 PDF」を生成。また、「Markdown」、「HTML」、「JSON」形式も合せて出力し、フルテキストインデックス作成や AI 分析処理等に利用可能に**  
   ...
   **単に YomiToku OCR ソフトウェアを呼び出しているだけではありません。快適性のために、読書用に必要な PDF 内部メタデータ再適用も行なっています。** YomiToku OCR ソフトウェアを単純に呼び出すと、PDF に埋め込んだ前記のメタデータ (物理／論理ページ変換データなど) が消えてしまいます。本ツールは、YomiToku OCR ソフトウェアが生成する PDF に、再度、それらのメタデータを埋め込む処理を行ないます。これにより、上記で解説した快適な読書用 PDF で、かつ文字列が埋め込まれた検索可能 PDF を、自動出力することができます。
...
## 1.4. 本派生版 (NI_SuperBook_PDF_Converter / JJ1BBY) における変更点

ここから「1.4.」節は、**派生版の保守・追加開発者である JJ1BBY による追記**です。以降の「2.」節以下の本文は、再びオリジナル版の解説 (原作者: 登 大遊 氏) に戻ります。

本派生版は、登 大遊 氏のオリジナル版「DN_SuperBook_PDF_Converter」を元に、JJ1BBY が自分自身の読書環境 (特に電子ペーパー端末での読書) に合わせて追加開発したものです。オリジナル版の優れた設計・実装の上に成り立っており、原作者に深く感謝いたします。オリジナル版と同様、本派生版も [AGPLv3](LICENSE) で公開します。

以下が、派生開始時点のオリジナル版からの主な追加・変更点です。変更の詳細は、各ソースコードおよび本リポジトリのコミット履歴をご参照ください。

### 1.4.1. EPUB 形式での出力に対応 (電子ペーパー端末向け)

オリジナル版が出力する Markdown (`md_paged`) を元に、章・ページ単位の **EPUB ファイルを生成する機能**を追加しました。EPUB の生成には、新たに外部ツール [pandoc](https://pandoc.org/) を内部呼出ししています (導入手順は後述の「3.2.8. pandoc ディレクトリ」を参照)。

これは、E-Ink (電子ペーパー) 端末で、固定レイアウトの PDF ではなくリフロー型の EPUB として快適に読むことを目的とした機能です。

特に、**縦書き書籍の図版について、電子ペーパー端末向けの回転処理**を実装しています。これらの端末は、横長の JPEG 画像を自動的に縦向きへ回転して表示してしまうため、図版が意図しない向きになる問題がありました。本派生版では、図版画像を物理的に 90 度回転 (左回転) させた上で、EPUB 内の該当 `<img>` を `<svg><image>` 要素でラップする後処理を行ない、端末側の自動回転を回避して、図版を正しい向きで表示できるようにしています。

> **⚠️ 本 EPUB 出力機能は、電子ペーパー端末「XTEINK X3」での表示に最適化し、同端末で動作確認を行なっています。** 上記の SVG ラップによる図版回転処理も、XTEINK X3 の自動回転挙動に合わせて調整したものです。他の EPUB ビューアや E-Ink 端末では、図版の向きや表示が異なる場合があります。

### 1.4.2. 高精細日本語 OCR の reading_order (読み順) オプションの追加

OCR を有効にした際に、`md_paged` 生成時の **reading_order (読み順) を選択**できるようにしました。

- **`right2left` (縦組み固定):** 読み順を「右 → 左」に固定します。縦書き書籍向けです。
- **`auto` (自動判定):** 読み順をエンジンに自動判定させます。横書き書籍などに有効ですが、**ページのレイアウトによっては読み順を誤判定する場合があります**。縦書き書籍では `right2left` を明示的に指定するのが確実です。

あわせて、OCR 出力処理を行なう補助スクリプト (`SuperBookToolsApp/PythonScripts/yomitoku_multi_export.py`) を Python 側に追加しています。

**OCR 推論のマルチフォーマット一括 (並列) 出力による高速化:** YomiToku の標準 CLI は、1 回の実行で 1 フォーマットしか出力できず、`md` / `html` / `json` / 検索可能 PDF をそれぞれ得るには、重い GPU 推論 (文字検出・レイアウト解析・文字認識) をフォーマットごとに繰り返す必要がありました。本スクリプトは、**1 回の推論結果から全フォーマット (paged / combined) をまとめて出力**することで、この冗長な再推論を排除し、複数フォーマットを生成する場合の処理時間を大幅に短縮します。

### 1.4.3. ConvertPdf コマンドの対話プロンプト改善と「EPUB のみ再生成」モード

`ConvertPdf` コマンド実行時の対話プロンプトを改善しました。

- **OCR 実行確認:** `[Y/n/e=epub-only]` — Enter (空入力) でデフォルト Yes。`e` を入力すると後述の EPUB のみ再生成モードになります。
- **読み順の確認:** `[R/a]` — Enter で `right2left` (読み順を右→左に固定。縦組み向け)、`a` で `auto` (読み順を自動判定。誤判定の可能性あり)。

`e` (EPUB のみ再生成モード) では、PDF 鮮明化処理や OCR 処理をすべてスキップし、**過去に生成済みの `md_paged` から EPUB だけを素早く再生成**します。一度 OCR まで処理した書籍について、EPUB の生成設定だけを変えて作り直したい場合に便利です。

### 1.4.4. 一時ディレクトリを設定ファイル (appsettings.json) で指定可能に

本ソフトウェアは処理中に大量の一時ファイルを生成します。`SuperBookToolsApp/appsettings.json` の `TempRootPath` を設定することで、**一時ディレクトリの場所を任意のドライブ・パスに変更**できるようにしました (空欄の場合は既定の一時ディレクトリを使用します)。空き容量の大きいドライブを指定したい場合に便利です。

ローカル環境固有の設定は、同じ JSON 形式の `appsettings.local.json` をアプリ実行ディレクトリに作成することで上書きできます (`.gitignore` 対象のためリポジトリには含まれません)。これにより `appsettings.json` を変更・コミットすることなく、環境ごとのパスを設定できます。

### 1.4.5. 安定性・パフォーマンスの改善

実運用 (35 冊規模の本番スキャン PDF 一括変換) で判明した問題に対し、以下の修正を行ないました。

- **縦書き (right2left) OCR 実行時の figure_merge 未適用を修正:** 縦書き読み順 (`right2left`) では `md_paged` の YomiToku タスクが単独グループになり、`merge_figure_gap` による近接図版マージが適用されなかった不具合を修正しました。
- **近接図版マージ (figure_merge) の C# 側パラメータ未渡しを修正:** `PdfYomitokuOptions.MergeFigureGapPx` (デフォルト 100px) が job JSON の `merge_figure_gap` として Python スクリプトに渡されておらず、図版マージが常に無効になっていた不具合を修正しました。これにより、同一ページ内で 100px 以内に隣接する複数の図版 bbox が自動的に 1 つにまとめられ、挿絵が分割されて複数の小画像になる問題を軽減します。
- **ImageMagick による大規模 PDF 展開のハング対策:** 500 ページを超える PDF で並列展開数を制限し、ページ数に連動したタイムアウトを設定。また、Windows の Mark-of-the-Web (Zone.Identifier 代替データストリーム) が原因のハングを回避するため、これを除去する処理を追加。
- **RealESRGAN エンジンの共有:** 複数 PDF を一括処理する際、PDF ごとにモデルを再ロードしていたのを、全 PDF で 1 つのエンジンを共有するよう変更し、モデルロードのオーバーヘッドを削減。
- **RealESRGAN のパラメータ最適化:** タイルサイズや使用モデルを調整 (詳細は [`docs/RealESRGAN_Performance_Notes.md`](docs/RealESRGAN_Performance_Notes.md) を参照)。
- **RealESRGAN Python サーバスクリプトの GPU 処理高速化:** `inference_realesrgan_server.py` に I/O パイプライン化を実装しました。先読みスレッド (`cv2.imread`) / GPU 推論メインスレッド / 後書きスレッド (`cv2.imwrite`) を 3 段パイプラインで並列実行することで、GPU が画像 I/O を待たずに推論専念できるため、1 枚あたりの処理時間が短縮されます。

  **⚠️ 注意: `external_tools/` ディレクトリは `.gitignore` 対象のため、上記変更はリポジトリに含まれていません。** 新規セットアップ時は、最適化済みスクリプトを手動でコピーする必要があります。詳細は「**3.3. RealEsrgan ディレクトリの準備**」を参照してください。

- **RealESRGAN GPU 推論中の VRAM 累積防止:** 各ページの推論完了後に `torch.cuda.empty_cache()` を呼び出し、PyTorch のキャッシュアロケータが確保した VRAM を解放するよう変更しました。この処理がないと、多ページ PDF の処理中に VRAM の reserved 領域が増加し続け、物理 VRAM を超過した場合にシステム RAM への流出 (共有 GPU メモリの使用) が発生して処理速度が著しく低下します。

- **RealESRGAN GPU 処理と CPU 後処理の並列化 (パイプライン化):** RealESRGAN がチャンク (16 ページ単位) の処理を完了するたびに、完成済みページの傾き補正・色統計計算 (CPU 処理) を直ちに開始するよう変更しました。GPU 処理の完了を待たずに CPU 処理が並行して進むため、全チャンク完了後の待機時間が大幅に短縮されます。
...
- **CPU:** AMD Ryzen 7 5700X (8 コア)
- **メモリ:** 32 GB

> 以下の数値はすべて本環境での実測値であり、GPU・CPU・メモリ構成によって変動します。

**① RealESRGAN 1 枚あたり処理時間 (モデル・タイルサイズ変更による高速化)**

オリジナル版の既定設定 (モデル `x4plus` / Tile=512) から、本派生版の設定 (モデル `x2plus` / Tile=2048) へ変更したことで、**約 7.9 倍の高速化**を実測しました (計測日: 2026/06/10)。

| 設定 | 1 枚あたり | 32 枚あたり (平均) |
|------|-----------|------------------|
| オリジナル版相当 (`x4plus` / Tile=512) | 約 13.7 秒 | 219.3 秒 |
| **本派生版 (`x2plus` / Tile=2048)** | **約 1.74 秒** | **55.8 秒** |

**② エンドツーエンド処理時間の比較 (PDF 1 冊あたり)**

559 ページ規模の PDF 1 冊を処理した際の各フェーズの所要時間を、オリジナル版相当の設定と本派生版で比較した実測値を示します (計測日: 2026/06/12〜13)。

| フェーズ | オリジナル版相当 ※1 | 本派生版 |
|---------|-------------------|---------|
| ImageMagick PDF 展開 | 約 3 分 | 約 3 分 |
| RealESRGAN GPU 処理 (559 ページ / 18 チャンク) | **約 66 分** ※2 | **約 32 分** |
| GPU 完了後の傾き補正・後処理待機 | **約 15 分** | **約 2.5 分** |
| AI-OCR (YomiToku・有効時) | 約 41 分 | 約 41 分 |
| **合計 (OCR 含まず)** | **約 84 分** | **約 37.5 分** |

- ※1 オリジナル版相当: Python プロセスをチャンクごとに再起動 (モデル毎回再ロード)、`empty_cache()` なし、GPU/CPU 並列化なし
- ※2 OOM が発生して GPU メモリがシステム RAM に流出した場合は、さらに数倍以上に悪化します (実測で 1 チャンクあたり約 20 分 / 合計 6 時間超に達した例あり)

**GPU 後の後処理待機短縮の内訳:**

GPU 処理完了から CPU 後処理 (傾き補正・色補正・余白検出) の開始までの待機時間は、GPU/CPU パイプライン化により大幅に短縮されました。

| | GPU 完了 → 後処理開始 | 後処理 (559 ページ) 全体 |
|---|---|---|
| 並列化なし (旧) | 7 分 37 秒 | 約 9 分 55 秒 |
| **並列化あり (本派生版)** | **10 秒** | **約 2 分 26 秒** |

#### スペックの異なる GPU・メモリ環境での注意

本派生版の既定設定 (Tile=2048) は、**VRAM 16 GB 級の GPU を前提**としており、ピーク時に約 12.3 GB / 16 GB (約 75%) の VRAM を使用します。利用環境のスペックが異なる場合は、以下の点に注意してください。

- **VRAM 容量が小さい GPU (8 GB 級など):** Tile=2048 のままではメモリ不足 (OOM) で失敗する可能性が高いです。その場合は、`SuperBookToolsApp/appsettings.json` の `RealEsrganTile` を **1024 や 512 などに下げて**ください (アプリ再起動で即時反映、再ビルド不要)。タイルサイズを下げると VRAM 使用量は減りますが、タイル分割数が増えるぶん処理速度は低下します (上表の「約 7.9 倍」は得られなくなります)。

  ```json
  { "TempRootPath": "D:\\tmp_work", "RealEsrganTile": 1024 }
  ```
- **GPU 性能:** 上記の倍率・絶対秒数はハイエンド GPU (RTX 5070 Ti) での値です。スペックの異なる GPU では絶対的な処理時間や高速化倍率が変動します。GPU を持たない (CPU のみの) 環境では、実用的でないほど時間がかかります。
- **システムメモリ (RAM):** 本ソフトウェアは多数のページをメモリ上に展開するため、ページ数の多い PDF では数 GB ～ 数十 GB の空き RAM が必要になることがあります (オリジナル版「2.1. OS・HW 環境」も参照)。RAM 容量が小さい環境では、スワップによる速度低下や処理失敗が起こり得ます。
- **一時ディスク容量:** 処理中に大量の一時ファイルを生成します。空き容量の大きいドライブを `appsettings.json` の `TempRootPath` で指定することを推奨します (前述「1.4.4.」を参照)。

### 1.4.6. 本派生版の開発における生成 AI の利用について (Vibe coding)

本派生版で追加・変更したコード (EPUB 出力、reading_order オプション、`yomitoku_multi_export.py`、各種安定性・パフォーマンス修正など) は、**生成 AI とのいわゆる「Vibe coding」(対話的に AI へ実装を指示し、生成されたコードを検証・調整しながら進める開発スタイル) によって作成**したものです。したがって、コードには生成 AI による出力が多く含まれます。

オリジナル版でも一部に AI コード生成が利用されています (本家 README「[7. 開発にあたっての AI コーディングの利用について](#7-開発にあたっての-ai-コーディングの利用について)」を参照)。本派生版の追加分は、それをさらに進め、生成 AI を主体とした開発で実装しています。

### 1.4.7. 謝辞

本派生版は、以下の方々・プロジェクトの成果の上に成り立っています。深く感謝申し上げます。

- **[登 大遊 (dnobori) 氏](https://github.com/dnobori/DN_SuperBook_PDF_Converter)** — オリジナル版「DN_SuperBook_PDF_Converter」の原作者。本ソフトウェアの根幹となる設計・実装はすべて原作者によるものです。
- **[kotaro.kinoshita 氏](https://github.com/kotaro-kinoshita/yomitoku)** — 高精細日本語 AI-OCR エンジン「YomiToku」の作者。
- **[pandoc](https://pandoc.org/) (John MacFarlane 氏ほか)** — EPUB 生成に利用しているドキュメント変換ツール。
- その他、オリジナル版が内部利用している [Real-ESRGAN](https://github.com/xinntao/Real-ESRGAN)、[ImageMagick](https://imagemagick.org/)、[Ghostscript](https://www.ghostscript.com/)、[ExifTool](https://exiftool.org/)、[pdfcpu](https://pdfcpu.io/)、[qpdf](https://github.com/qpdf/qpdf)、[Tesseract OCR](https://github.com/tesseract-ocr/) の各プロジェクト。

なお、本派生版に対するご意見・派生開発については、原作者の方針 (前掲「8. 拡張開発されたい方へ」) と同様に、本リポジトリへのプルリクエストではなく、各自で Fork して独自に開発・公開していただく形を推奨します。
...
- GPU: 内部で利用している [RealEsrgan](https://github.com/xinntao/Real-ESRGAN) という画像鮮明化 AI が、GPU 処理を必要とします。CPU でも処理はできると思いますが、その場合、極めて長時間かかり、実用的ではありません。GPU は、NVIDIA の CUDA 対応の GeForce シリーズを推奨します。

  **CUDA・PyTorch バージョンについて:**
  - 以下のインストール手順では `--index-url https://download.pytorch.org/whl/nightly/cu128` を使用します。これは **CUDA 12.8 対応の PyTorch nightly (開発版)** を意味します。
  - **`cu128` = CUDA 12.8** です。CUDA 12.8 を使用するには、NVIDIA ドライバが対応している必要があります。GeForce RTX 5000 番台 (Blackwell アーキテクチャ) の場合はゲームレディドライバ **576.x 以降**が必要です。RTX 4000 番台以前でも、CUDA 12.x 対応の比較的新しいドライバであれば動作する可能性があります。
  - **`nightly` ビルドが必要な理由:** 2026 年現在、RTX 5000 番台 (Blackwell) は PyTorch の安定版 (stable) ではサポートされていないため、nightly ビルドを使用します。RTX 4000 番台以前の GPU では、nightly の代わりに stable の `--index-url https://download.pytorch.org/whl/cu128` でも動作する可能性があります。
  - CUDA 以外 (AMD GPU / Intel GPU 等) でも動作するかも知れませんが、未確認です。
...
- Python 3 系を利用します。RealESRGAN と YomiToku でそれぞれ独立した venv を作成します。
  - **RealESRGAN 用 venv:** Python **3.10.x** を使用します。basicsr ライブラリの互換性の関係で、3.10 系が安定しています (3.11 以降は動作しないか、追加の修正が必要になる場合があります)。以下の手順では Python 3.10.11 を例として使用します。
  - **YomiToku 用 venv:** Python **3.11.x** を使用します。以下の手順では Python 3.11.9 を例として使用します。
...
```
"C:\Program Files\Git\bin\git.exe" clone --recursive https://github.com/JJ1BBY/NI_SuperBook_PDF_Converter.git
```
...
### 3.2.7. TesseractOCR_Data ディレクトリ
...
### 3.2.8. pandoc ディレクトリ
**(本派生版で追加。EPUB 出力機能を使用する場合のみ必要です。)**

[pandoc](https://pandoc.org/) の 3.x 系 Windows x64 版を入れます。このディレクトリの直下に、`pandoc.exe` というファイルが置かれた状態にしてください。本ドキュメント作成時は pandoc 3.10 を使用しています。このソフトウェアはフリーソフトウェア (GPLv2 以降) なので、インターネット上からダウンロードすることが可能です。以下からダウンロードできます。  
[https://github.com/jgm/pandoc/releases](https://github.com/jgm/pandoc/releases)

Windows 用の ZIP 版 (例: `pandoc-3.x-windows-x86_64.zip`) を展開し、その中の `pandoc.exe` を、この `pandoc` ディレクトリの直下にコピーしてください。本ソフトウェアは、OCR 結果の Markdown から EPUB を生成する際に、pandoc を子プロセスとして呼び出します。



## 3.3. external_tools\external_tools\image_tools\RealEsrgan\ ディレクトリの準備

1. [Python 3.10.11 for Windows](https://www.python.org/ftp/python/3.10.11/python-3.10.11-amd64.exe) をインストールします。**RealESRGAN は basicsr ライブラリの互換性のため Python 3.10.x を使用します** (3.11 以降は追加の修正が必要になる場合があります)。  
   インストール後、`%LOCALAPPDATA%\Programs\Python\Python310\python.exe` に `python.exe` が存在するという前提で、以下の解説をいたします。
1. コマンドプロンプトを開きます。
1. 以下を 1 つずつ実行します。意味をよく理解しながら、実行してください。環境によっては、追加的に補正作業が必要となる可能性があります。
   ```
   mkdir <このgitをダウンロードしたディレクトリ>\NI_SuperBook_PDF_Converter\external_tools\external_tools\image_tools\RealEsrgan\RealEsrgan_Repo\

   cd /d <このgitをダウンロードしたディレクトリ>\NI_SuperBook_PDF_Converter\external_tools\external_tools\image_tools\RealEsrgan\RealEsrgan_Repo\
   ...
   cd /d <このgitをダウンロードしたディレクトリ>\NI_SuperBook_PDF_Converter\external_tools\external_tools\image_tools\RealEsrgan\RealEsrgan_Repo\Real-ESRGAN\

   "C:\Program Files\Git\bin\git.exe" checkout a4abfb2979a7bbff3f69f58f58ae324608821e27
   ```
1. [https://github.com/xinntao/Real-ESRGAN/releases/download/v0.1.0/RealESRGAN_x4plus.pth](https://github.com/xinntao/Real-ESRGAN/releases/download/v0.1.0/RealESRGAN_x4plus.pth) を、`<このgitをダウンロードしたディレクトリ>\NI_SuperBook_PDF_Converter\external_tools\external_tools\image_tools\RealEsrgan\RealEsrgan_Repo\Real-ESRGAN\weights\` に、手動でダウンロードして、保存します。
1. 以下を 1 つずつ実行します。意味をよく理解しながら、実行してください。環境によっては、追加的に補正作業が必要となる可能性があります。
   ```
   cd /d <このgitをダウンロードしたディレクトリ>\NI_SuperBook_PDF_Converter\external_tools\external_tools\image_tools\RealEsrgan\RealEsrgan_Repo\
   ...
   ```
1. 任意のテキストエディタで、`<このgitをダウンロードしたディレクトリ>\NI_SuperBook_PDF_Converter\external_tools\external_tools\image_tools\RealEsrgan\RealEsrgan_Repo\venv\Lib\site-packages\basicsr\data\degradations.py` を開きます。  
   以下のとおり、インチキ書き換えをします。  
   旧:
   ```
   ...
   ```
1. 任意のテキストエディタで、`<このgitをダウンロードしたディレクトリ>\NI_SuperBook_PDF_Converter\external_tools\external_tools\image_tools\RealEsrgan\RealEsrgan_Repo\Real-ESRGAN\realesrgan\version.py` というファイル名の、内容が空のファイルを作成します。

1. **最適化済み RealESRGAN サーバスクリプトのコピー (推奨)**  
   `external_tools/` ディレクトリは `.gitignore` 対象のため、GPU 処理を最適化した `inference_realesrgan_server.py` はリポジトリに含まれていません。本ファイルを `docs/` に保存してあるので、以下のコマンドで上書きコピーしてください。

   ```
   copy /Y "<このgitをダウンロードしたディレクトリ>\NI_SuperBook_PDF_Converter\docs\inference_realesrgan_server.py" "<このgitをダウンロードしたディレクトリ>\NI_SuperBook_PDF_Converter\external_tools\external_tools\image_tools\RealEsrgan\RealEsrgan_Repo\Real-ESRGAN\inference_realesrgan_server.py"
   ```

   このスクリプトには I/O パイプライン化 (先読み/GPU推論/書き込みの3段並列) と VRAM 解放処理が含まれており、オリジナルスクリプトと比べて RealESRGAN の処理速度・安定性が向上します (詳細は「**1.4.5. 安定性・パフォーマンスの改善**」を参照)。このコピーを省略した場合、オリジナルの逐次処理スクリプトで動作します。
...
なお、YomiToku は、[kotaro.kinoshita](https://github.com/kotaro-kinoshita/) 氏が公開・配布されているソフトウェアです。無償でダウンロードでき、「非商用での個人利用・研究目的での利用」は「自由に行っていただけます」と記載されています。しかし、商用利用等にあたっては、ライセンスの購入が必要と記載されています。詳しくは、同ソフトウェアの [README.md] (https://github.com/kotaro-kinoshita/yomitoku/blob/cba0a134e0d2ad3bfdce163231b3cb91de07928e/README.md) をご参照ください。
...
2. 以下を 1 つずつ実行します。意味をよく理解しながら、実行してください。環境によっては、追加的に補正作業が必要となる可能性があります。
   ```
   cd /d <このgitをダウンロードしたディレクトリ>\NI_SuperBook_PDF_Converter\external_tools\external_tools\image_tools\yomitoku\
...
### 4.2.3. スキャンし変換した書籍 PDF は、原則、個人的利用に限定
...
# 5. (参考) 著作権者不明等の場合の裁定制度 (古い書籍をデジタル化して電子出版するビジネスが可能)
...
# 6. お勧め PDF ビューアの耳より情報
## 6.1. iPhone / iPad / macOS 版 PDF ビューアの最高峰は「PDF Viewer by Nutrient」だと思われる
...
## 6.2. Windows 版 PDF ビューアの最高峰は「Sumatra PDF」だと思われる (一部改造推奨)
