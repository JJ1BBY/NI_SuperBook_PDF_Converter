...
特に、**縦書き書籍の図版について、電子ペーパー端末 (XTEINK X3 等) 向けの回転処理**を実装しています。これらの端末は、横長の JPEG 画像を自動的に縦向きへ回転して表示してしまうため、図版が意図しない向きになる問題がありました。本派生版では、図版画像を物理的に 90 度回転 (左回転) させた上で、EPUB 内の該当 `<img>` を `<svg><image>` 要素でラップする後処理を行ない、端末側の自動回転を回避して、図版を正しい向きで表示できるようにしています。
