...
本派生版の既定設定 (Tile=2048) は、**VRAM 16 GB 級の GPU を前提**としており、ピーク時に約 12.3 GB / 16 GB (約 75%) の VRAM を使用します。利用環境が非力な場合は、以下の点に注意してください。

- **VRAM が少ない GPU (8 GB 級など):** Tile=2048 のままではメモリ不足 (OOM) で失敗する可能性が高いです。その場合は、`internal_libs/.../AppLib/Misc/AiUtil.cs` の `AiUtilRealEsrganPerformOption.Tile` の値を **1024 や 512 などに下げて**ください。タイルサイズを下げると VRAM 使用量は減りますが、タイル分割数が増えるぶん処理速度は低下します (上表の「約 7.9 倍」は得られなくなります)。
- **GPU 性能:** 上記の倍率・絶対秒数はハイエンド GPU (RTX 5070 Ti) での値です。より非力な GPU では絶対的な処理時間が長くなり、高速化倍率も変動します。GPU を持たない (CPU のみの) 環境では、実用的でないほど時間がかかります。
- **システムメモリ (RAM):** 本ソフトウェアは多数のページをメモリ上に展開するため、ページ数の多い PDF では数 GB ～ 数十 GB の空き RAM が必要になることがあります (オリジナル版「2.1. OS・HW 環境」も参照)。RAM が少ない環境では、スワップによる極端な速度低下や処理失敗が起こり得ます。
