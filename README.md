...
- **読み順の確認:** `[R/a]` — Enter で `right2left` (縦組み)、`a` で `auto` (横組み)。
...
> 上記の高速化倍率・秒数は本環境での実測値であり、GPU・CPU・メモリ構成によって変動します。VRAM は本設定 (Tile=2048) でピーク約 12.3 GB / 16 GB を使用します。VRAM が少ない環境では Tile サイズの調整が必要な場合があります。
