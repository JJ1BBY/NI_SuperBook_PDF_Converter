...
1. **最適化済み RealESRGAN サーバスクリプトのコピー (推奨)**  
   ...
   このスクリプトには I/O パイプライン化と `torch.compile` ウォームアップが含まれており、オリジナルスクリプトと比べて RealESRGAN の処理速度が向上します (詳細は「**1.4.5. 安定性・パフォーマンスの改善**」を参照)。このコピーを省略した場合、オリジナルの逐次処理スクリプトで動作します。
